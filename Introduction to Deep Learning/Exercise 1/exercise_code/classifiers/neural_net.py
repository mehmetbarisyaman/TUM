...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        first_layer_result = X.dot(W1) + b1
        relu_result = np.maximum(0, first_layer_result)
        scores = relu_result.dot(W2) + b2
        ...
        reg_add = 0.5 * reg * np.sum(np.square(W1)) + 0.5 * reg * np.sum(np.square(W2))
        loss = data_loss + reg_add
        ########################################################################
        ...
        grads["W2"] = np.dot(relu_result.T, dscores)
        grads["b2"] = np.sum(dscores, axis=0)
        
        dhidden = np.dot(dscores, W2.T)
        dhidden[relu_result<=0] = 0
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            X_batch = None
            y_batch = None
            minibatch_indices = np.random.choice(len(y), batch_size)
    ...
    def predict(self, X):
        ...
        """
        y_pred = None
        first_result = X.dot(self.params["W1"]) + self.params["b1"]
        relu = np.maximum(0, first_result)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for lr in learning_rates:
        for rs in regularization_strengths:
            neural_net = TwoLayerNet(input_size, hidden_size, 10)
            stats = neural_net.train(X_train, y_train, X_val, y_val, num_iters=iters, batch_size=200, learning_rate = lr, learning_rate_decay = 0.95, reg=rs)
